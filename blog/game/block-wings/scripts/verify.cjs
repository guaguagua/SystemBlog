...
(async()=>{
 ...
 await page.goto('file:///'+root.replaceAll('\\','/')+'/index.html');assert.equal(await page.locator('#levelSelect button').count(),5);await page.locator('#start').click();assert.deepEqual(await page.evaluate(()=>clips),[]);
 ...
 const mobile=await browser.newPage({viewport:{width:390,height:844},isMobile:true,hasTouch:true});mobile.on('pageerror',e=>errors.push(e.message));await mobile.goto('file:///'+root.replaceAll('\\','/')+'/index.html');await mobile.locator('#sound').tap();await mobile.locator('#start').tap();await mobile.touchscreen.tap(90,600);await mobile.screenshot({path:path.join(root,'../../../tmp/block-magic-mobile.png'),fullPage:true});assert.ok(await mobile.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 // Decode every generated MP3 through the real browser audio pipeline.
 const audioPage=await browser.newPage();await audioPage.goto('file:///'+root.replaceAll('\\','/')+'/index.html');const durations=await audioPage.evaluate(async()=>{const files=BLOCK_CURRICULUM.levels.flatMap(l=>l.words.flatMap(e=>['cn','en','sentenceCn','sentenceEn'].map(f=>`assets/speech/${e.id}-${f}.mp3`)));return await Promise.all(files.map(src=>new Promise(resolve=>{const a=new Audio(src);a.onloadedmetadata=()=>resolve(a.duration);a.onerror=()=>resolve(-1);setTimeout(()=>resolve(-1),10000);})));});assert.equal(durations.length,200);assert.ok(durations.every(d=>Number.isFinite(d)&&d>0));
